...
public class ServiceCompra : IServiceCompra
{
    ...
    {
        if (dto.Items is null || dto.Items.Count == 0) throw new InvalidOperationException("Sin items.");
        ...
        foreach (var it in dto.Items)
        {
            ...
            if (lote is null) throw new InvalidOperationException("Lote inválido.");
            var ahora = DateTime.UtcNow;
            var vigente = lote.VigenteDesde <= ahora && (lote.VigenteHasta == null || ahora <= lote.VigenteHasta);
            if (!vigente) throw new InvalidOperationException($"Lote {lote.Nombre} fuera de vigencia.");
            if (lote.Vendidas + it.Cantidad > lote.Cupo) throw new InvalidOperationException($"Cupo insuficiente en {lote.Nombre}.");
            ...
            for (int i = 0; i < it.Cantidad; i++)
            {
                entradas.Add(new Entrada {
                    EventoId = it.EventoId, LoteEntradaId = it.LoteId, Codigo = Guid.NewGuid().ToString("N").ToUpperInvariant()
                });
            }
        ...
        var compra = new Compra {
            UsuarioId = dto.UsuarioId, EmailContacto = dto.Email, DireccionEnvio = dto.DireccionEnvio, MetodoPago = dto.MetodoPago,
            Total = total, Estado = "Creado"
        };
        _db.Compras.Add(compra);
        await _db.SaveChangesAsync();

        foreach (var en in entradas) { en.CompraId = compra.Id; _db.Entradas.Add(en); }
        await _db.SaveChangesAsync();

        await tx.CommitAsync();

        return new CompraDTO(
            compra.Id, compra.EmailContacto, compra.Creada, compra.Total, compra.Estado,
            entradas.Select(e => new EntradaDTO(e.Id, e.EventoId, e.LoteEntradaId, e.Codigo, e.Validada, e.FechaValidacion)).ToList()
        );
    }
